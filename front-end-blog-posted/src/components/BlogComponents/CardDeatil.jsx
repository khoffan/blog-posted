...
export default function CardDeatil({ id }) {
    ...
    const getProfileuser = async () => {
        try {
            const response = await axios.get(
                `${import.meta.env.VITE_BASE_API_URI}/api/profile`,
                {
    ...
    const invertTime2Datetime = (isotime) => {
        ...
        let localDate = date.toLocaleString("th-TH", {
    ...
    return (
        <>
            {hasBlog ? (
                <>
                    <div className="flex flex-col justify-start items-start w-3/4 h-full mt-6 ">
                        <p className="text-2xl font-bold text-start">
                            {blogDeatil.title}
                        </p>
                        <div className="flex items-start justify-between gap-4 w-3/4 h-auto px-4">
                            <div className="flex flex-col items-center ">
                                <div className="flex items-center">
                                    <img
                                        ...
                                        className="w-[30px] h-[30px] mx-2 bg-transparent rounded-full object-cover"
                                    />
                                    <p className="text-sm text-bold">
                                        {blogDeatil.author.name}
                                    </p>
                                </div>
                                <p className="text-sm text-thin text-gray-400">
                                    {blogDeatil.author.email}
                                </p>
                            </div>

                            <p className="text-sm opacity-25">
                                created at:{" "}
                                {invertTime2Datetime(blogDeatil.createdAt)}
                            </p>
                        </div>
                        ...
                        <div className="w-[75%] min-h-[200px] mb-2 px-4 border border-1 border-red-500">
                            <p className="px-[0.5rem] text-xl mb-[1rem]">
                                Comments
                            </p>
                            <form
