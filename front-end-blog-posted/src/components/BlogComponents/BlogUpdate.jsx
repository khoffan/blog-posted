...
export default function BlogUpdate() {
	...
	const { currentBlog, fetchBlogById, updateBlog, uploadBlogImage, isLoading } = useBlogStore();
	...
	useEffect(() => {
		if (currentBlog && currentBlog._id === id) {
			if (currentBlog.content && currentBlog.content.length > 0) {
                ...
                setParagraphs([{ type: "text", content: currentBlog.title }, { type: "text", content: currentBlog.description }]);
	...
	const handleUpdateBlog = async () => {
		...
		if (result.success) {
			if (tags.length > 0) {
				await Promise.all(
					tags.map((tagname) => createTag(id, tagname))
				);
			}
